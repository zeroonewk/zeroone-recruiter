...
export async function PATCH(request: NextRequest, { params }: RouteContext) {
  try {
    ...
    if ('default_days_offset' in body) {
      ...
      setClauses.push(`default_days_offset = $${i++}`);
      values.push(rawOffset);
    }
    ...
    if (setClauses.length === 0) {
      const rows = (await sql`
        SELECT id, name, position, default_days_offset, is_archived, created_at, updated_at
        FROM workflow_stages_template WHERE id = ${id}
      `) as StageRow[];
      if (!rows[0]) {
        return NextResponse.json({ ok: false, error: 'Etap nie istnieje' }, { status: 404 });
      }
    ...
    values.push(id);
    const queryStr = `UPDATE workflow_stages_template SET ${setClauses.join(', ')} WHERE id = $${i} RETURNING id, name, position, default_days_offset, is_archived, created_at, updated_at`;
...
export async function DELETE(_req: NextRequest, { params }: RouteContext) {
  try {
    ...
    const rows = (await sql`
      UPDATE workflow_stages_template SET is_archived = TRUE WHERE id = ${id}
      RETURNING id
    `) as { id: string }[];
