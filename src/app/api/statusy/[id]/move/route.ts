...
type StatusBasic = { id: string; position: number; is_archived: boolean };
...
export async function POST(request: NextRequest, { params }: RouteContext) {
  try {
    ...
    if (current.is_archived) {
      return NextResponse.json(
        { ok: false, error: 'Nie mozna przesuwac zarchiwizowanego statusu' },
        { status: 400 }
      );
    }
    ...
    await sql`
      UPDATE result_statuses SET position = ${neighbor.position} WHERE id = ${id}
    `;
    await sql`
      UPDATE result_statuses SET position = ${current.position} WHERE id = ${neighbor.id}
    `;
